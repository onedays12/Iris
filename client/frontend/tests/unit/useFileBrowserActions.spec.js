...
} from '../../src/composables/useFileBrowserActions.js'
...
describe('useFileBrowserActions.buildCopyName', () => {
  it('appends _copy before extension for a normal file', () => {
    expect(buildCopyName({ name: 'a.txt' })).toBe('a_copy.txt')
    expect(buildCopyName({ name: 'report.pdf' })).toBe('report_copy.pdf')
  })

  it('appends _copy as suffix for a directory (no extension handling)', () => {
    expect(buildCopyName({ name: 'mydir', is_dir: true })).toBe('mydir_copy')
  })

  it('appends _copy as suffix for a file with no extension', () => {
    // dotIndex <= 0 branch — Makefile has no dot.
    expect(buildCopyName({ name: 'Makefile' })).toBe('Makefile_copy')
    expect(buildCopyName({ name: 'README' })).toBe('README_copy')
  })

  it('handles dotfiles correctly (leading dot does NOT count as extension)', () => {
    // .gitignore: lastIndexOf('.') === 0, dotIndex <= 0 → suffix _copy.
    expect(buildCopyName({ name: '.gitignore' })).toBe('.gitignore_copy')
    expect(buildCopyName({ name: '.bashrc' })).toBe('.bashrc_copy')
  })

  it('handles multi-dot filenames by splitting at the LAST dot', () => {
    // archive.tar.gz → archive.tar_copy.gz
    expect(buildCopyName({ name: 'archive.tar.gz' })).toBe('archive.tar_copy.gz')
    expect(buildCopyName({ name: 'a.b.c.txt' })).toBe('a.b.c_copy.txt')
  })

  it('returns "Copy" when name is empty', () => {
    expect(buildCopyName({ name: '' })).toBe('Copy')
  ...
  it('ignores is_dir when name is empty (falls to "Copy")', () => {
    expect(buildCopyName({ name: '', is_dir: true })).toBe('Copy')
  })
  ...
  it('handles names with dots only at start AND no extension', () => {
    ...
    expect(buildCopyName({ name: '..hidden' })).toBe('._copy.hidden')
  })
})
