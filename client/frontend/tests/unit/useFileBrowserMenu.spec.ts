...
import { useFileBrowserMenu } from '../../src/composables/useFileBrowserMenu'
import type { FileBrowserMenuActions, FileMenuTarget } from '../../src/composables/useFileBrowserMenu'
...
function mountMenu(currentPathValue = '') {
  ...
  let result!: ReturnType<typeof useFileBrowserMenu>
...
describe('useFileBrowserMenu.getMenuTarget', () => {
  ...
  it('returns { type: "folder", file } for a directory', () => {
    const { wrapper, result } = mountMenu('C:\\current')
    const folder = { name: 'mydir', is_dir: true } as ExplorerFileInfo
  ...
  it('returns { type: "file", file } for a file (is_dir falsy)', () => {
    const { wrapper, result } = mountMenu('C:\\current')
    const file = { name: 'a.txt', is_dir: false } as ExplorerFileInfo
  ...
  it('returns { type: "file", file } when is_dir is undefined', () => {
    const { wrapper, result } = mountMenu('C:\\current')
    const file = { name: 'a.txt' } as ExplorerFileInfo
    expect(result.getMenuTarget(file)).toEqual({ type: 'file', file })
    wrapper.unmount()
  })
...
describe('useFileBrowserMenu.closeMenu / openMenu', () => {
  it('closeMenu sets activeMenuTarget to null', () => {
    const { wrapper, result } = mountMenu()
    result.activeMenuTarget.value = { type: 'file', file: { name: 'a' } as ExplorerFileInfo }
    result.closeMenu()
  ...
  it('openMenu sets activeMenuTarget to the given target and places the menu', () => {
    const { wrapper, result } = mountMenu()
    const target: FileMenuTarget = { type: 'file', file: { name: 'a.txt' } as ExplorerFileInfo }
...
describe('useFileBrowserMenu.handleMenuAction', () => {
  function mountWithActions(actions: Partial<FileBrowserMenuActions>) {
  ...
  it('does nothing when target is null/undefined', () => {
    ...
    expect(() => result.handleMenuAction('download', null as unknown as FileMenuTarget)).not.toThrow()
    expect(() => result.handleMenuAction('download', undefined as unknown as FileMenuTarget)).not.toThrow()
  ...
  it.each([
    ...
    const { wrapper, result } = mountWithActions(actions)
    const file = { name: 'a.txt' } as ExplorerFileInfo
    const target: FileMenuTarget = { type: 'file', file }
