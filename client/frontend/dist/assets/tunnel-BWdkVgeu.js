import { a1 as request, a3 as defineStore, a4 as pick, a5 as toNumber } from "./index-CTSqJF0U.js";
async function listTunnels(page = 1, pageSize = 20) {
...
async function updateTunnel(tunnelId, payload) {
  return await request("PATCH", `/api/v1/tunnels/${encodeURIComponent(tunnelId)}`, payload);
}
async function listTunnelChannels(tunnelId, page = 1, pageSize = 20) {
...
function normalizePagePayload(payload) {
  ...
  const items = Array.isArray(payload == null ? void 0 : payload.items) ? payload.items : Array.isArray(payload == null ? void 0 : payload.Items) ? payload.Items : [];
  return {
    items,
...
function sameTunnel(left, right) {
  ...
  return Boolean(
    left.beaconId && right.beaconId && left.type === right.type && left.bindHost === right.bindHost && left.bindPort === right.bindPort && left.remoteHost === right.remoteHost && left.remotePort === right.remotePort && left.socksAuthMode === right.socksAuthMode && left.socksUdpAssociate === right.socksUdpAssociate
  );
}
const useTunnelStore = defineStore("tunnel", {
  state: () => ({
    tunnels: [],
    loading: false,
    error: "",
  ...
  actions: {
    async fetchTunnels({ silent = false } = {}) {
      ...
      try {
        ...
        const list = [];
        while (true) {
          const data = await listTunnels(page, pageSize);
    ...
    async createTunnel(payload) {
      const result = await createTunnel(payload);
      await this.fetchTunnels({ silent: true }).catch(() => {
      });
      return result;
    },
    async updateTunnel(tunnelId, payload) {
      const result = await updateTunnel(tunnelId, payload);
      await this.fetchTunnels({ silent: true }).catch(() => {
      });
      return result;
    },
    async pauseTunnel(tunnelId) {
      const result = await pauseTunnel(tunnelId);
      await this.fetchTunnels({ silent: true }).catch(() => {
      });
      return result;
    },
    async resumeTunnel(tunnelId) {
      const result = await resumeTunnel(tunnelId);
      await this.fetchTunnels({ silent: true }).catch(() => {
      });
      return result;
    },
    async clearTunnel(tunnelId) {
      const result = await clearTunnel(tunnelId);
      this.removeTunnelLocal(tunnelId);
      return result;
    },
    async recycleTunnelChannels(tunnelId, recycledCount = 0) {
      const result = await recycleTunnelChannels(tunnelId, recycledCount);
    ...
    async fetchChannels(tunnelId, { silent = false } = {}) {
      ...
      try {
        ...
        const list = [];
        while (true) {
          const data = await listTunnelChannels(key, page, pageSize);
          const pageData = normalizePagePayload(data);
          list.push(...pageData.items);
          if (!pageData.hasMore || pageData.items.length === 0) break;
          page += 1;
